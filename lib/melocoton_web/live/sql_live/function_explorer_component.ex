defmodule MelocotonWeb.SqlLive.FunctionExplorerComponent do
  use MelocotonWeb, :live_component
  alias Melocoton.DatabaseClient

  @impl true
  def update(%{repo: repo, function_id: id} = assigns, socket) do
    socket
    |> assign(assigns)
    |> assign(:result, DatabaseClient.get_function_definition(repo, id))
    |> ok()
  end
  ...
  def render(assigns) do
    ...
    <div class="flex-1 flex flex-col overflow-hidden" style="background: var(--bg-primary);">
      ...
      >
        ...
        />
        <span style="color: var(--text-secondary);">{@function_name}</span>
        <span class="mx-1.5" style="color: var(--text-tertiary);">·</span>
        <span class="uppercase tracking-wide" style="color: var(--text-tertiary);">
          {@function_kind}
        </span>
      </div>
